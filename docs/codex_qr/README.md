...
To speed up this key entry process we have defined a way to encode a BIP-93 codex32 string as a QR code that can be instantly scanned into Bails or potentially any other Bitcoin hardware wallet that has a camera.
...
<table align="center">
    <tr><td><img src="img/handmade_qr.jpg"><br/><em>Obviously this CodexQR is just for demonstration purposes; never photograph your CodexQRs!</em></td></tr>
</table>


We have defined two QR formats:
* Standard Codex32 QR
* And a more advanced CompactCodexQR

Specifications for each follow below, as well as discussion of the pros and cons of each format.



## Quick Review of BIP-93 Codex32 Seed Backups
The best method for backing up a Bitcoin wallet is to store its [BIP-93](https://github.com/bitcoin/bips/blob/master/bip-0093.mediawiki) codex32 backup consisting of 48 or 74 character strings.

Each character comes from the [bech32 character set](https://github.com/bitcoin/bips/blob/master/bip-0173.mediawiki#user-content-Bech32).
...
```bash
# original 128-bit codex32 secret:
"ms10testsxxxxxxxxxxxxxxxxxxxxxxxxxx4nzvca9cmczlw"

# final assembled codex32 QR:
"MS10TESTSXXXXXXXXXXXXXXXXXXXXXXXXXX4NZVCA9CMCZL"

# original 256-bit codex32 string:
...
This alphanumeric string is then encoded into a QR code.
...
QR codes are typically used to encode a website url, in which case the "Alphanumeric" format has to be used (the encoded data can consist of uppercase letters, numbers, and certain allowed symbols).
...
Notice that the "Numeric" and "Alphanumeric" columns have greater capacity. This is because when you have fewer possible characters to encode, it takes less data to specify each one.

Alphanumeric format supports 45 possible characters. They are encoded as pairs, so that means there are 45^2 = 2,025 possible pairings to describe. Each pair can then be encoded using 11 bits:

```
2^11 = 2048
```

If you only used 10 bits, you could only specify 2^10 = 1,024 possible values; not nearly enough for the 2,025 possible pairings. 11 bits is the best this format can do.
...
Armed with this knowledge, we're ready to encode our CodexQR.
...
# `CompactCodexQR` Specification
The `CompactCodexQR` format builds upon the "Standard" `CodexQR` format by further optimizing how the data is stored in order to generate smaller QR codes that are easier to transcribe by hand.

Notice in the QR code capacity chart above that there's also a "Binary" or "Numeric" column. "Binary" capacity count initially looks pretty unimpressive; a 25x25 QR code is limited to 32.

But here the unit being described isn't alphanumeric characters; it's max number of bytes.

```
1 byte = 8 bits
```

Rather than having the QR format interpret our data as numbers or characters, we can directly encode the relevant bits that determine our codex32 seed backup.
